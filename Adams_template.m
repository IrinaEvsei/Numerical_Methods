...
for m=1:3
   k1 = f(t(m), U1(:, m)).*h;
   k2 = f(t(m) + h / 2, U1(:, m) + k1 / 2).*h;
   k3 = f(t(m) + h / 2, U1(:, m) + k2 / 2).*h;
   k4 = f(t(m) + h, U1(:, m) + k3).*h;
  
   U1(:, m + 1) = U1(:, m) + 1/6 * (k1 + 2 * k2 + 2 * k3 + k4);  
...
%следующие узлы
 ...
 figure
 plot(ts,U1(1,:),'-o',t,y(:,1),'--',ts,yTochn,'-',ts,U(1,:),'-.b','linewidth',1)
